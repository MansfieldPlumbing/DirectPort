...
void Log(const std::wstring& msg) {
    ...
    wsprintfW(buffer, L"[PID:%lu][SuperConsumer64] %s\n", pid, msg.c_str());
    OutputDebugStringW(buffer);
}

// This manifest structure is compatible with both producer types.
...
static ComPtr<ID3D11Device5>          g_device5;      // For opening fences
static ComPtr<ID3D11DeviceContext>    g_context;
static ComPtr<ID3D11DeviceContext4>   g_context4;     // For waiting on fences
...
// --- Multi-Producer Resource Management ---
...
struct ProducerConnection {
    ...
    ComPtr<ID3D11Texture2D>        privateTexture;   // Local copy for rendering
    ComPtr<ID3D11ShaderResourceView> privateSRV;
    UINT                           connectedTextureWidth = 0;
    UINT                           connectedTextureHeight = 0;
    std::wstring                   producerType; // "D3D11" or "D3D12"
};
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    const WCHAR szClassName[] = L"SuperConsumer64WindowClass";

    // --- FIX: Use WNDCLASSEXW to support both big and small icons ---
    ...
    // --- FIX: Load icons and assign them to the window class ---
    wcex.hIcon = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(SC64_ICON1), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
    wcex.hIconSm = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(SC64_ICON1), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_SHARED);
    
    RegisterClassExW(&wcex); // --- FIX: Use RegisterClassExW ---

    HWND hwnd = CreateWindowExW(0, szClassName, L"SuperConsumer64 - Searching...", WS_OVERLAPPEDWINDOW,
        CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, nullptr, nullptr, hInstance, nullptr);

    if (!hwnd) return 1;

    // --- FIX: Explicitly set the icons for the window instance to guarantee they appear ---
...
void CreatePrivateTextureAndSRV(int producerIndex, UINT width, UINT height, DXGI_FORMAT format) {
    ...
    desc.MipLevels = 1;
    desc.ArraySize = 1;
    // --- FIX --- Use the format provided by the producer's manifest
    desc.Format = format; 
    ...
    if (SUCCEEDED(g_device->CreateTexture2D(&desc, nullptr, &producer.privateTexture))) {
        // The SRV can also use the same format
...
void RenderFrame() {
    ...
    // --- FIX --- Changed clear color to "No Signal" blue.
    const float clearColor[] = { 0.0f, 0.2f, 0.4f, 1.0f }; 
    g_context->ClearRenderTargetView(g_rtv.Get(), clearColor);

    RECT clientRect;
    GetClientRect(FindWindowW(L"SuperConsumer64WindowClass", NULL), &clientRect);
    ...
    for (int i = 0; i < MAX_PRODUCERS; ++i) {
        ...
        if (producer.isConnected && producer.sharedSRV && producer.privateSRV) {
            ...
            D3D11_VIEWPORT vp = {};
            vp.Width = windowWidth / (float)MAX_PRODUCERS;
            vp.Height = windowHeight;
            vp.TopLeftX = i * (windowWidth / (float)MAX_PRODUCERS);
            vp.TopLeftY = 0.0f;
            vp.MinDepth = 0.0f;
            vp.MaxDepth = 1.0f;
        ...
        do {
            ...
            for (const auto& sig : producerSignatures) {
                ...
                if (memcmp(&pManifestView->adapterLuid, &g_adapterLuid, sizeof(LUID)) == 0) {
                    ...
                    producer.producerType = sig.second;
                    ...
                    // --- FIX --- Pass the format from the manifest when creating the private texture
                    CreatePrivateTextureAndSRV(availableSlot, desc.Width, desc.Height, pManifestView->format);
                    
                    Log(L"Slot " + std::to_wstring(availableSlot) + L": Connected to " + sig.second + L" producer (PID: " + std::to_wstring(pe32.th32ProcessID) + L")");
                    UpdateWindowTitle();
...
void UpdateWindowTitle() {
    std::wstring title = L"SuperConsumer64 - ";
    ...
    SetWindowTextW(FindWindowW(L"SuperConsumer64WindowClass", NULL), title.c_str());
}
