...
void Log(const std::wstring& msg) { WCHAR b[512]; wsprintfW(b, L"[TegrityCamD3D12] %s\n", msg.c_str()); OutputDebugStringW(b); }
void LogHRESULT(const std::wstring& msg, HRESULT hr) { WCHAR b[512]; wsprintfW(b, L"[TegrityCamD3D12] %s - HRESULT: 0x%08X\n", msg.c_str(), hr); OutputDebugStringW(b); }
...
// This struct MUST match the vertex layout in your C++ code.
struct VSInput
{
...
// This struct is the output of the vertex shader.
struct PSInput
{
...
// This is the correct "passthrough" vertex shader.
// It uses the data from the vertex buffer and does NOT generate its own geometry.
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    g_sharedTextureName = L"Global\\TegrityCamD3D12Texture_" + std::to_wstring(pid);
    g_sharedFenceName = L"Global\\TegrityCamD3D12Fence_" + std::to_wstring(pid);
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow) {
    ...
    wcex.lpszClassName = L"D3D12CameraApp";

    // --- FIX: Load and assign the icons to the window class ---
    wcex.hIcon = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(IDI_CAMICON), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
    wcex.hIconSm = (HICON)LoadImageW(hInstance, MAKEINTRESOURCEW(IDI_CAMICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_SHARED);

    if (!RegisterClassExW(&wcex)) return E_FAIL;

    g_hWnd = CreateWindowW(L"D3D12CameraApp", L"D3D12 Camera (Click-Cycle Cam, Wheel-Cycle Res)", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, nullptr, nullptr, hInstance, nullptr);
    if (!g_hWnd) return E_FAIL;

    // --- FIX: Explicitly set the icons on the window handle to guarantee they appear ---
...
void PopulateCommandList() {
    ...
    if (g_bIsCameraStreamReady && read_slot != g_q_write_idx.load(std::memory_order_acquire)) {
        ...
        if (isNV12) {
            CD3DX12_RESOURCE_BARRIER preBarriers[] = {
                CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureY.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_DEST),
                CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureUV.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_DEST)
            };
            g_commandList->ResourceBarrier(_countof(preBarriers), preBarriers);
        } else if (g_pCameraTextureY) {
            g_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureY.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_DEST));
        }
        ...
        if (isNV12) {
            ...
            // FIX START: Calculate the required size for the Y plane and use it as the offset for the UV plane.
            UINT64 totalSize = 0;
            ...
            CD3DX12_TEXTURE_COPY_LOCATION DstY(g_pCameraTextureY.Get(), 0);
            CD3DX12_TEXTURE_COPY_LOCATION SrcY(g_uploadHeap.Get(), yLayout);
            g_commandList->CopyTextureRegion(&DstY, 0, 0, 0, &SrcY, nullptr);

            CD3DX12_TEXTURE_COPY_LOCATION DstUV(g_pCameraTextureUV.Get(), 0);
            CD3DX12_TEXTURE_COPY_LOCATION SrcUV(g_uploadHeap.Get(), uvLayout);
            g_commandList->CopyTextureRegion(&DstUV, 0, 0, 0, &SrcUV, nullptr);

        } else if (g_pCameraTextureY) {
            D3D12_SUBRESOURCE_DATA yuy2Data = { frame, W * 2, W * 2 * H };
            UpdateSubresources(g_commandList.Get(), g_pCameraTextureY.Get(), g_uploadHeap.Get(), 0, 0, 1, &yuy2Data);
        }

        if (isNV12) {
            CD3DX12_RESOURCE_BARRIER postBarriers[] = {
                 CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureY.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
                 CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureUV.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)
            };
            g_commandList->ResourceBarrier(_countof(postBarriers), postBarriers);
        } else if (g_pCameraTextureY) {
             g_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(g_pCameraTextureY.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
        }
    ...
    if (g_sharedTexture) {
        g_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(g_sharedTexture.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_RENDER_TARGET));
        RenderCameraScene(g_sharedRtvHeap->GetCPUDescriptorHandleForHeapStart(), g_currentSharedSize, g_currentSharedSize, false);
        g_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(g_sharedTexture.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON));
    }
...
void InternalChangeSharedResolution(BOOL bCycleUp) {
    ...
    std::wstring title = L"D3D12 Camera (Shared: " + std::to_wstring(newSize) + L"x" + std::to_wstring(newSize) + L" " + resType + L")";
